...
def load_all_ip_to_asn_sources(ip_version=4, tags='default'):
    ...
    directory = root_dir / 'stats/ip2as_data/'
...
def load_submarine_and_asn_data():
    ...
    save_file = root_dir / 'stats/submarine_data/landing_points_dict'
    ...
    save_file = root_dir / 'stats/asnLinks.jsonl'
    ...
    for count, (org, cables) in enumerate(submarine_owners.items()):
        ...
        for cable in cables:
            ...
            for landing_point in cable_dict[cable].landing_points:
                landing_point_coordinates = (
                landing_points_dict[landing_point].latitude, landing_points_dict[landing_point].longitude)
...
def check_name_overlap(submarine_org, caida_org_list):
    if len(submarine_org.split()) == 1:
        match = [item for item in caida_org_list if
                 submarine_org.lower() in [i.lower().strip(',') for i in item.split()]]
    else:
...
def get_best_fit_caida_org_name_for_submarine_owner():
    ...
    caida_org_to_rank_dict, caida_org_to_asn_dict, caida_org_to_country_map, caida_asn_name_to_rank_dict, caida_asn_name_to_org_dict = generate_as_mapping_based_on_caida_asrank_data(
        asn_data)

    # Computing the short forms for all CAIDA orgs before hand, rather than repeated computation
    caida_org_to_short_form, caida_short_form_to_org = compute_short_forms_for_orgs_list(
        list(caida_org_to_rank_dict.keys()))

    # Generating the cleaned org names for all CAIDA orgs
    caida_org_to_clean_org, caida_clean_org_to_org = clean_given_org_list(list(caida_org_to_asn_dict.keys()),
                                                                          stopwords=stopwords, most_common=None)
    ...
    for count, org in enumerate(submarine_owners_ranked.keys()):
        ...
        if '(' in org:
            org_parts = [strip_punctuations_and_accents(item.strip(')').strip(), stopwords=stopwords) for item in
                         org.split('(')]
            check = True
        ...
        if len(all_matches) > 0:
            ...
            if best_match[1] < 5000:
                ...
                asn_list = generate_asn_list_given_best_org_match(best_match[0], list(match_rank_sorted.keys()),
                                                                  asn_to_customer_asn_map, caida_org_to_asn_dict)
            ...
            if cleaned_org_new == cleaned_org and len(cleaned_org_new.split()) == 1:
                ...
                for cleaned_org in org_parts:
                    ...
                    if len(match) > 0:
                        match = [caida_clean_org_to_org[item] for item in match]
                        all_matches_failure.update(match)
                        examine_matched_orgs_validity(match, submarine_org_to_country_map[org],
                                                      caida_org_to_country_map)
                        all_matches.update(match)
