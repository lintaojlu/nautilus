...
def load_mapping_file(suffix='default'):
    mapping_file = f'stats/mapping_outputs_{suffix}/link_to_cable_and_score_mapping_sol_validated_v4'
...
if __name__ == '__main__':
    ...
    print(
        f'Overlap between latency link and our mapping : {len(overlap)}. Total latency links : {len(latency_link_to_measurement_map)}')

    print('Loading ip to country cluster map')
    with open(f'stats/mapping_outputs_{suffix}/geolocation_country_cluster_sol_validated_v4', 'rb') as fp:
                        ...
                        if mapping_original_link[1] == mapping_latency_link[1]:
                            ...
                            if len(overlap_cables) > 0:
                                ...
                                if mapping_original_link[1][0] == mapping_latency_link[1][0]:
                                    ...
                                    omin = min(
                                        [max(mapping_original_link[1].index(item), mapping_latency_link[1].index(item))
                                         for item in overlap_cables])
                                    partial_common.append(omin)
                                print(
                                    f'Cables for original: {mapping_original_link[1]}, latency link: {mapping_latency_link[1]} and matched cables are : {overlap_cables}')
                            else:
                                if len(mapping_latency_link[1]) > 0 and len(mapping_original_link[1]) > 0:
                                    # Taking the top country pairs
                                    original_link_country = (
                                    country_mapping[original_link[0]][0][0], country_mapping[original_link[1]][0][0])
                                    latency_link_country = (
                                    country_mapping[latency_link[0]][0][0], country_mapping[latency_link[1]][0][0])
                                    ...
                                    # no_cable_match += 1
                                    print(
                                        f'No match: Cables for original: {mapping_original_link[1]}, latency link: {mapping_latency_link[1]}')
                                else:
                                    ignore_count += 1
                                    print(
                                        f'Looks like no mapping was made. Cables for original: {mapping_original_link[1]}, latency link: {mapping_latency_link[1]}')
